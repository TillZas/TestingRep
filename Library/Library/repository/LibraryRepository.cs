...
namespace Library.Models
{
    ...
    public class LibraryRepository : ILibraryRepository
    {
        ...
        public Book GetBook(int id)
        {
            using (IDbConnection db = new SqlConnection(connectionString))
            {
                ...
                return db.Query<Book, Author, Book>(sql,
                    (bk, ath) =>
                    {
                        bk.Author = ath == null ? new Author() : ath;
                        return bk;
                    },
        ...
        public List<Book> FindBooksByTitleAndAuthor(int author,String name)
        {
            using (IDbConnection db = new SqlConnection(connectionString))
            {
                return db.Query<Book, Author, Book>(
                    "SELECT * FROM Books AS b\n" +
                    "LEFT JOIN Authors AS a on a.AuthorId = b.AuthorRefId\n" +
                    "WHERE b.Title LIKE '%" + name + "%' AND b.AuthorRefId = "+author,
                    (bk, ath) =>
                    {
                        bk.Author = ath == null ? new Author() : ath;
                        return bk;
        ...
        public class AutorEnum
        {
            public int? value { get; set; }
            public string text { get; set; }
        }

        public List<AutorEnum> getAutorsAsEnum(bool addNullable)
        {
            List<AutorEnum> result = new List<AutorEnum>();
            if (addNullable)
                result.Add(new AutorEnum() { value = null, text = "Без автора" });

            List<Author> autors = GetAuthors();

            foreach(Author at in autors)
            {
                result.Add(new AutorEnum() { value = at.AuthorId, text = at.Name});
            }
        ...
        public List<string> GetAllBookNames()
        {
            ...
            List<Book> bks = GetBooks();
            foreach(Book bk in bks)
            {
                if (bk.Title != null) res.Add(bk.Title);
            }
